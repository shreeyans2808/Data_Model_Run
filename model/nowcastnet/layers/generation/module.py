...
class GenBlock(nn.Module):
    def __init__(self, fin, fout, evo_ic, use_se=False, dilation=1, double_conv=False):
        super().__init__()
...
class SPADE(nn.Module):
    def __init__(self, norm_nc, label_nc):
        ...
        self.mlp_shared = nn.Sequential(
            nn.ReflectionPad2d(pw), nn.Conv2d(label_nc, nhidden, kernel_size=ks, padding=0), nn.ReLU()
        )
